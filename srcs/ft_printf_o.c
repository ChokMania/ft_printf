...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/23 12:05:15 by anmauffr          #+#    #+#             */
/*   Updated: 2019/01/25 12:43:37 by judumay          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ftprintf.h"

static __uint64_t	ft_printf_o_get_arg(t_printf *p)
{
	__uint64_t	ret;
...
static t_printf		*ft_printf_o_precision(t_printf *p, __uint64_t tmp)
{
	size_t		tmp2;
	char		*str;
	char		*buf;
	...
	{
		if (p->flags->hash && (tmp || (!tmp && !p->precision)))
			--tmp2;
...
static t_printf		*ft_printf_o_champ(t_printf *p)
{
	size_t		tmp;
	char		*str;
	char		*buf;
...
static t_printf		*ft_printf_o_flags(t_printf *p, __uint64_t tmp)
{
	char		*str;
	char		*buf;
